...
return static function (): void {
    ...
    test_assert_same('confirmation', $expense['status'] ?? null, 'Every expense must wait for explicit confirmation.');
    ...
    test_assert_equals(1000.0, finance_load_set($db, 7, 'accounts')[0]['saldo'], 'A preview must not update the selected account balance.');
    $confirmedExpense = $service->resolveConfirmation(7, (string)$expense['actionToken'], 'confirm');
    test_assert_same('applied', $confirmedExpense['status'] ?? null, 'The confirmed expense must be applied.');
    test_assert_equals(957.10, finance_load_set($db, 7, 'accounts')[0]['saldo'], 'Confirmation must update the selected account balance.');
    ...
    test_assert_same('confirmation', $income['status'] ?? null, 'Every income must wait for explicit confirmation.');
    test_assert_same('avulso', $income['data']['type'] ?? null, 'A one-off received amount must be categorized as avulso.');
    test_assert_equals(10.0, finance_load_set($db, 9, 'accounts')[0]['saldo'], 'An income preview must not change the selected account balance.');
    $confirmedIncome = $service->resolveConfirmation(9, (string)$income['actionToken'], 'confirm');
    test_assert_same('applied', $confirmedIncome['status'] ?? null, 'The confirmed income must be applied.');
    test_assert_equals(50.0, finance_load_set($db, 9, 'accounts')[0]['saldo'], 'Confirmed income must increase the selected account balance.');
    ...
    $undoneIncome = $service->undo(9, (string)$confirmedIncome['actionToken']);
    ...
    test_assert_same('confirmation', $namedIncome['status'] ?? null, 'A named income must still wait for confirmation.');
    test_assert_same('Next - CC', $namedIncome['data']['account'] ?? null, 'A unique bank token must resolve safely among multiple accounts.');
    test_assert_equals(0.0, finance_load_set($db, 10, 'accounts')[0]['saldo'], 'The named account must remain unchanged before confirmation.');
    $confirmedNamedIncome = $service->resolveConfirmation(10, (string)$namedIncome['actionToken'], 'confirm');
    test_assert_same('applied', $confirmedNamedIncome['status'] ?? null, 'The named income must apply after confirmation.');
    ...
    $triage = $service->handle(7, 'request_confirmation_0001', 'Transfira R$ 200 da Conta A para a Conta B', null);
    test_assert_same('routed', $triage['status'] ?? null, 'The general agent must only route the request.');
    test_assert_same('financeiro', $triage['module'] ?? null, 'The general agent must choose the finance specialist.');
    test_assert_same('route_to_agent', $triage['action'] ?? null, 'The general agent must never return a mutation action.');
    test_assert_same(0, $triage['usage']['totalTokens'] ?? null, 'General triage must not call an external provider.');
    test_assert_same(0, $provider->calls, 'General triage must consume no provider calls.');
    test_assert_equals(957.10, finance_load_set($db, 7, 'accounts')[0]['saldo'], 'General triage must not alter data.');
    $generalHistory = $repository->history(7, 'geral');
    test_assert_same('routed', $generalHistory[0]['response']['status'] ?? null, 'General triage history must remain isolated.');

    $preview = $service->handle(7, 'request_confirmation_finance_0001', 'Transfira R$ 200 da Conta A para a Conta B', 'financeiro');
    ...
    $financeHistory = $repository->history(7, 'financeiro');
    $cancelledHistory = array_values(array_filter(
        $financeHistory,
        static fn(array $item): bool => ($item['requestId'] ?? null) === 'request_confirmation_finance_0001',
    ));
    test_assert_same('cancelled', $cancelledHistory[0]['response']['status'] ?? null, 'History must stay isolated in the routed finance agent and reflect cancellation.');
